...
# ОС (Linux / Darwin) — на macOS штатный bash 3.2 и BSD-утилиты, флаги отличаются
OS="$(uname -s 2>/dev/null || echo Linux)"

# Портативный таймаут: GNU timeout / gtimeout (macOS coreutils) / чистый bash-фолбэк.
# На macOS команды timeout по умолчанию нет.
TIMEOUT_CMD=""
...
with_timeout() {
  # with_timeout <сек> <команда...>  → код возврата команды.
  # Таймаут отражается кодом 124 (GNU) либо >=128 (фолбэк, убито сигналом).
  local t="$1"; shift
...
resolve_system() {
  # Разрешение штатным резолвером ОС (портативно: Linux + macOS)
  local h="$1"
  if have getent; then
    getent ahostsv4 "$h" 2>/dev/null | awk '{print $1}' | grep -E '^[0-9]+(\.[0-9]+){3}$' | sort -u
  elif have dscacheutil; then                       # macOS
    dscacheutil -q host -a name "$h" 2>/dev/null | awk '/^ip_address:/{print $2}' | grep -E '^[0-9]+(\.[0-9]+){3}$' | sort -u
  elif have dig; then
    dig +short A "$h" 2>/dev/null | grep -E '^[0-9]+(\.[0-9]+){3}$' | sort -u
...
PY
    ...
    host "$h" 2>/dev/null | awk '/has address/{print $NF}' | sort -u
  fi
}
...
if [[ -n "$IP" ]]; then
  # флаг таймаута ожидания у ping отличается: Linux -W сек, macOS -W мс
  if [[ "$OS" == "Darwin" ]]; then ping_wait=(-W 2000); else ping_wait=(-W 2); fi
  if ping -c 3 "${ping_wait[@]}" "$IP" >"/tmp/xba_ping.$$" 2>&1; then
  ...
  if [[ "$HAVE_TRACEROUTE" -eq 1 ]]; then
    log_info "Трассировка маршрута до $IP:$PORT..."
    tr_out=""
    # TCP-трассировка на целевой порт (флаг -T есть только у Linux-traceroute)
    if [[ "$IS_ROOT" -eq 1 && "$OS" != "Darwin" ]]; then
      tr_out="$(traceroute -n -T -p "$PORT" -w 2 -q 1 -m 20 "$IP" 2>/dev/null)"
    fi
    # фолбэк: обычная traceroute (на macOS -w в секундах — оставляем совместимо)
    ...
    if [[ -n "$tr_out" ]]; then
      ...
      if [[ -n "$reached" ]]; then
        ...
        lh="$(echo "$LAST_HOP" | awk '{print $2}')"
        if [[ -n "$lh" ]]; then
          log_warn "Трассировка НЕ дошла до целевого IP — пакеты гибнут по пути."
          log_dim "Последний ответивший хоп: $lh"
        else
          log_warn "Трассировка без ответов (все хопы '*') — узлы по пути режут ICMP/UDP-пробы."
          log_dim "Это частая ситуация и сама по себе НЕ означает блокировку (см. TCP ниже)."
        fi
      fi
    else
...
tcp_probe() {
  ...
  elif [[ $rc -eq 124 || $rc -ge 128 ]]; then echo "timeout"   # 124=GNU timeout, >=128=убит фолбэком
...
#   (без ассоциативных массивов — совместимо с bash 3.2 на macOS)
# ---------------------------------------------------------------------------
section "3. Транспортный уровень (L4 / TCP)"
h_connected=0; h_timeout=0; h_reset=0
if [[ -n "$IP" ]]; then
  log_info "TCP-пробы на $IP:$PORT (попыток: $ATTEMPTS)..."
  for i in $(seq 1 "$ATTEMPTS"); do
    r="$(tcp_probe "$IP" "$PORT" 5)"
    case "$r" in
      connected) h_connected=$((h_connected + 1)) ;;
      timeout)   h_timeout=$((h_timeout + 1)) ;;
      reset)     h_reset=$((h_reset + 1)) ;;
    esac
    log_dim "попытка $i: $r"
  done
  # Доминирующий исход
  ...
  for k in connected timeout reset; do
    case "$k" in
      connected) n=$h_connected ;;
      timeout)   n=$h_timeout ;;
      reset)     n=$h_reset ;;
    esac
    (( n > bestn )) && { best="$k"; bestn=$n; }
  done
  case "$best" in
    connected) log_ok  "TCP-хендшейк УСПЕШЕН ($h_connected/$ATTEMPTS). L4 до IP:порт открыт."; V_TCP="ok" ;;
    timeout)   log_err "TCP-хендшейк: ТАЙМАУТ ($h_timeout/$ATTEMPTS). SYN уходит без ответа — пассивный дроп."; V_TCP="timeout" ;;
    reset)     log_err "TCP-хендшейк: RST/REFUSED ($h_reset/$ATTEMPTS). Активный сброс соединения."; V_TCP="reset" ;;
  esac
  log_dim "итог: connected=$h_connected timeout=$h_timeout reset=$h_reset"
...
if [[ "$IS_ROOT" -eq 1 && "$HAVE_TCPDUMP" -eq 1 && -n "$IP" ]]; then
  cap="/tmp/xba_cap.$$"
  # Интерфейс: на Linux работает 'any', на macOS нужен конкретный (берём дефолтный маршрут)
  td_if="any"
  if [[ "$OS" == "Darwin" ]]; then
    td_if="$(route -n get default 2>/dev/null | awk '/interface:/{print $2}')"
    [[ -z "$td_if" ]] && td_if="en0"
  fi
  # Слушаем SYN-ACK и RST от сервера в отдельном процессе
  tcpdump -n -i "$td_if" -c 40 -v "host $IP and tcp port $PORT and (tcp[tcpflags] & (tcp-rst) != 0 or tcp[tcpflags] & (tcp-syn) != 0)" \
      >"$cap" 2>/dev/null &
  tdpid=$!
  sleep 0.5
...
tls_probe() {
  ...
  # Флаги минимальны для совместимости с LibreSSL (openssl на macOS):
  # -brief/-verify_quiet там не поддерживаются.
  local ip="$1" port="$2" sni="$3" out rc
  local args=(s_client -connect "$ip:$port")
  [[ -n "$sni" ]] && args+=(-servername "$sni")
  out="$(printf 'Q\n' | with_timeout 10 openssl "${args[@]}" 2>&1)"
  rc=$?
  # Признаки успешного рукопожатия (есть и в OpenSSL, и в LibreSSL)
  if echo "$out" | grep -qiE 'Cipher is|Cipher +:|Protocol +:|Server certificate|Server public key|SSL-Session|Verify return code|handshake has read'; then
    echo "ok"
  elif [[ $rc -eq 124 || $rc -ge 128 ]]; then
    echo "timeout"
  else
    # reset by peer / handshake failure / refused / no route и т.п.
    echo "reset"
  fi
}
