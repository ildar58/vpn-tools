...
## Монитор адаптивной блокировки (поведение ТСПУ)

Файл: `xray-block-monitor.sh`

Анализатор выше даёт **срез в моменте**. Но ТСПУ часто блокирует не сразу:
какое-то время флоу «наблюдается», затем `IP:порт` уходит в блок на
фиксированные **~5–10 минут** (penalty-таймер), после чего разблокируется — и
цикл повторяется. Такое ловится только **наблюдением во времени**.

Монитор непрерывно зондирует ноду и строит «осциллограмму» доступности,
замеряя: **time-to-block** (сколько работает до блока), **длительность блока**
(кучность у 5–10 мин = penalty-таймер), **период** рабочих окон, **тип блока**
(таймаут = пассивный blackhole vs RST = активный сброс) и параллельно пробит
**контрольный хост** (точечный блок ноды или общий сбой канала).

> Если блок триггерится **объёмом трафика**, есть два пути: запускать монитор
> одновременно с реальным использованием ноды, **либо** добавить флаг `--load`
> — монитор сам нагрузит ноду HTTPS-трафиком (несколько параллельных воркеров,
> с корректным SNI — для Reality это тот же паттерн, что видит ТСПУ) и покажет,
> через сколько (и после какого объёма) под нагрузкой нода «гаснет».

### Быстрый запуск

```bash
curl -fsSL https://raw.githubusercontent.com/ildar58/vpn-tools/main/xray-block-monitor.sh | bash -s -- node.example.com --tls
```

### Запуск

```bash
# базовый мониторинг час, проба каждые 5с
bash xray-block-monitor.sh node.example.com

# с TLS-пробой (ловит блок по SNI), бесконечно, с CSV-логом
bash xray-block-monitor.sh node.example.com:2053 --tls --duration 0 --log run.csv

# самому нагрузить ноду, чтобы спровоцировать volume-триггер ТСПУ
bash xray-block-monitor.sh node.example.com --tls --load --load-workers 4
```

Опции: `--interval N`, `--duration N` (0 = до Ctrl-C), `--tls`, `--load`,
`--load-workers N`, `--sni <name>`, `--control <host[:port]>`, `--log <file>`.
Итоги печатаются при выходе (Ctrl-C).
